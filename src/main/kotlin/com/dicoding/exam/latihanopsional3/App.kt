...
fun manipulateString(str: String, int: Int): String {
    ...
    val stringTempt = str.filter { elem -> elem.isLetter() }
    val stringTempLength = stringTempt.length
    if (stringTempLength == lengthStr) {
        return "$str$int"
    }

    val intTempt = str.takeLast(lengthStr - stringTempLength).toInt() * int
    return "$stringTempt$intTempt"
}
